...
from .models import Employee, Customer, InventoryItem, Expense, Payment, StockHistory


@admin.register(Employee)
class EmployeeAdmin(admin.ModelAdmin):
    list_display = ['employee_id', 'name', 'position', 'department', 'salary', 'status', 'join_date']
    list_filter = ['status', 'department', 'join_date']
    search_fields = ['name', 'employee_id', 'position', 'phone', 'email']
    list_per_page = 20
    ordering = ['-join_date']
    
    fieldsets = (
        ('Basic Information', {
            'fields': ('name', 'employee_id', 'position', 'department')
        }),
        ('Contact Details', {
            'fields': ('email', 'phone', 'address')
        }),
        ('Employment Details', {
            'fields': ('salary', 'join_date', 'status')
        }),
        ('Additional Notes', {
            'fields': ('notes',),
            'classes': ('collapse',)
